...
class FileProcessor:
    ...
    def process_date(self):
        ...
        for file in self.data_dir.glob('*'):
            # 只处理文件,不处理文件夹
            if file.is_file() and file.suffix.lower() == '.csv':
                filename_lower = Path(file).stem.lower()
                if 'bg' in filename_lower or 'background' in filename_lower:
                    # 读取背景数据，确保数据类型为float64
                    background_data = pd.read_csv(file, header=None, 
                                                names=['wavelength', 'background_intensity'],
                                                dtype={'wavelength': float, 'background_intensity': float})
                else:
                    # 读取文件数据，确保数据类型为float64
                    df = pd.read_csv(file, header=None, 
                                   names=['wavelength', Path(file).stem],
        ...
        # 使用更安全的列操作方式进行除法运算
        background_values = result['background_intensity'].values
        for column in result.columns[2:]:  # 从第三列开始
            result[column] = result[column].values / background_values
